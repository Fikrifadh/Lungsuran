...
const DEFAULT_MESSAGE = `Halo Admin Lungsuran! 👋
...
export default async function SettingsPage() {
  const [phoneRow, messageRow] = await Promise.all([
    ...
    <div className="flex min-h-screen bg-slate-50 font-sans">
      ...
      <aside className="w-64 bg-[#00519E] text-white flex flex-col shadow-xl shrink-0">
        ...
        <nav className="flex-1 p-4 space-y-1 mt-4">
          ...
          <Link href="/admin/categories" className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 rounded-xl font-medium text-white/80 transition-all">
          ...
          <Link href="/admin/settings" className="flex items-center gap-3 px-4 py-3 bg-white/10 rounded-xl font-bold transition-all">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-20 bg-white border-b border-slate-200 flex items-center px-8 shrink-0">
          <div>
            <h1 className="text-2xl font-black text-slate-800">Pengaturan WhatsApp</h1>
            <p className="text-slate-500 text-sm">Atur nomor dan template pesan untuk tombol "Beli via WhatsApp".</p>
          </div>
        </header>
        <div className="flex-1 overflow-y-auto p-8">
          <WASettingsClient
            initialPhone={phoneRow?.value ?? ""}
            initialMessage={messageRow?.value ?? DEFAULT_MESSAGE}
          />
        </div>
      </main>
    </div>
